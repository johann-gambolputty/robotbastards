	...
	public class ClientUpdateManager : Scene.ISceneObject
	{
		...
		private void OnReceivedServerMessage( IConnection connection, Message msg )
		{
			...
			foreach ( UpdateMessage updateMsg in serverMsg.UpdateMessages )
			{
				...
				IClientUpdater updater = ( IClientUpdater )m_UpdaterIdMap[ updateMsg.TargetId ];
				updater.HandleClientUpdate( updateMsg );
			}
		}
		...
		private void OnTick( Scene.Clock clock )
		{
			...
			uint oldestClientSequence = uint.MaxValue;
			...
			for ( int clientIndex = 0; clientIndex < m_Clients.Count; ++clientIndex )
			{
				...
				foreach ( IClientUpdater curUpdater in m_Updaters )
				{
					...
					curUpdater.GetUpdateMessages( messages, curConnection.CurrentSequence, m_Sequence );
				}
				
				if ( messages.Count > 0 )
				{
					ClientMessage clientMsg = new ClientMessage( m_Sequence, ( UpdateMessage[] )messages.ToArray( typeof( UpdateMessage ) ) );

					//	Deliver the message to the client
					curConnection.Connection.DeliverMessage( clientMsg );
		...
		public void AddedToScene( Scene.SceneDb db )
		{
			...
			foreach ( IConnection connection in connections.AllConnections )
			{
				...
				{
					OnNewClientConnection( connection );
				}
			}
