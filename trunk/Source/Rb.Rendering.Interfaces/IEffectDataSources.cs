...
namespace Rb.Rendering.Interfaces
{
	...
	public interface IEffectDataSources
	{
		...
		/// <summary>
		/// Binds a data source to parameters in any effect with a given semantic tag. Invokes <see cref="DataSourceAdded"/>
		/// </summary>
		/// <param name="semantic">Semantic value</param>
		/// <returns>Returns a new IEffectValueDataSource used to set and get the parameter's value</returns>
		IEffectValueDataSource<T> CreateValueDataSourceForSemantic<T>( string semantic );
