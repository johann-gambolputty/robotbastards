...
{
	/// <summary>
	/// Invokes a visitor (<see cref="IGraphNodeVisitor"/>) for each node in a graph
	/// </summary>
	public class GraphNodeWalker
	{
		/// <summary>
		/// Visits all the nodes in a graph. Used when the maximum node identifier is known
		/// </summary>
		...
		/// </remarks>
		public static void Walk( IEnumerable<IGraphNode> startNodes, IGraphNodeVisitor visitor, int maxNodeId )
		{
			if ( startNodes == null )
			{
			...
			Queue<IGraphNode> nodeQueue = new Queue<IGraphNode>( );
			...
			try
			{
				...
				{
					IGraphNode node = nodeQueue.Dequeue( );
		...
		/// A node is only visited once all its input nodes have been visited first.
		/// </remarks>
		public static void Walk( IGraphNode[] startNodes, IGraphNodeVisitor visitor )
		{
		...
		/// </summary>
		private static void GrowNodeStatusArrayToIncludeNode( IGraphNode node, ref NodeStatus[] nodeStatuses )
		{
		...
		/// </summary>
		private static void SetNodeStatus( IGraphNode node, NodeStatus status, ref NodeStatus[] nodeStatuses )
		{
		...
		/// </summary>
		private static bool CheckNodeStatus( Queue<IGraphNode> nodeQueue, IGraphNode node, ref NodeStatus[] nodeStatuses )
		{
			bool canVisit = true;
			foreach ( IGraphNode inputNode in node.InputNodes )
			{
		...
		/// </summary>
		private static void Enqueue( Queue<IGraphNode> nodeQueue, IGraphNode node, ref NodeStatus[] nodeStatuses )
		{
			nodeQueue.Enqueue( node );
		...
		/// </summary>
		private static void Enqueue( Queue<IGraphNode> nodeQueue, IEnumerable<IGraphNode> nodes, ref NodeStatus[] nodeStatuses )
		{
