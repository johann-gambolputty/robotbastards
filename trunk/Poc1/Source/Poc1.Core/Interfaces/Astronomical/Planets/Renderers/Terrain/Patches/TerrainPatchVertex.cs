...
namespace Poc1.Core.Interfaces.Astronomical.Planets.Renderers.Terrain.Patches
{
	...
	{
		/// <summary>
		/// Copies this patchVertex to another, applying a specified offset to the patchVertex position
		/// </summary>
		public unsafe void CopyTo( TerrainPatchVertex* patchVertex, Vector3 offset )
		{
			patchVertex->m_X = m_X + offset.X;
			patchVertex->m_Y = m_Y + offset.Y;
			patchVertex->m_Z = m_Z + offset.Z;

			patchVertex->m_NormalX = m_NormalX;
			patchVertex->m_NormalY = m_NormalY;
			patchVertex->m_NormalZ = m_NormalZ;

			patchVertex->m_U = m_U;
			patchVertex->m_V = m_V;

			patchVertex->m_Slope = m_Slope;
			patchVertex->m_Elevation = m_Elevation;
		}
